...
export const fetchPoolCommits: (pool: string, network: SourceType,
	params: {
		from?: number, to?: number
	}
) => Promise<APICommitReturn[]> = async (pool, network, {
	from,
	to
}) => {
	const commits: APICommitReturn[] = await fetch(
			`${BASE_REPUTATION_API}/commits?source=${SourceMap[network]}&from=${from ?? 0}&to=${to ?? Math.round(Date.now() / 1000)}&pool_address=${pool.toLowerCase()}`
		)
		.then((res) => res.json())
		.then((commits) => {

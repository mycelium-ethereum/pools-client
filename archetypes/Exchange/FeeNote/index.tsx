...
const FeeNote: React.FC<FeeNoteProps> = ({ pool, isMint }) => {
    return (
        <HiddenExpand defaultHeight={0} open={!!pool.name} className="text-cool-gray-600 my-5 text-sm">
            <b>Note</b>: A small keeper fee will be taken when you {isMint ? 'receive' : 'sell'} the token in{' '}
            <TimeLeft nextRebalance={pool.lastUpdate.plus(pool.updateInterval).toNumber()} />. To learn more, visit{' '}
            <a
